...
def validate(specification_file):
    # Open Specification
    if isinstance(specification_file, file):
        specification_json = json.load(specification_file)
    elif isinstance(specification_file, (str, unicode)):
        specification_json = json.loads(specification_file)
    elif isinstance(specification_file, dict):
        specification_json = specification_file
    else:
    ...
    for file_info in file_infos:
        ...
        for url in file_info[URL_SOURCES]:
            md5 = get_md5(url, file_info[FILE_SIZE])

            if md5 and md5 != file_info[MD5]:
                pass


def get_md5(the_file_or_url, file_size, call_back_function=None, *args):
    if isinstance(the_file_or_url, file):
        return calculate_md5_via_file(the_file_or_url, file_size, call_back_function, *args)
    elif isinstance(the_file_or_url, (str, unicode)):
        return calculate_md5_via_url(the_file_or_url, file_size, call_back_function, *args)
    else:
        raise ValueError('the_file_or_url must be a file or a string form of a url')


def calculate_md5_via_url(url, supposed_file_size, call_back_function, *args):
    ...
    return calculate_md5(remote, file_size, supposed_file_size, call_back_function, *args)


def calculate_md5_via_file(the_file, file_size, call_back_function, *args):
    if not isinstance(the_file, file):
        raise ValueError('the_file must be an open file ')

    return calculate_md5(the_file, file_size, file_size, call_back_function, *args)


def calculate_md5(data_source, actual_file_size, supposed_file_size, call_back_function, *args):
    bytes_downloaded = 0
    md5 = hashlib.md5()
